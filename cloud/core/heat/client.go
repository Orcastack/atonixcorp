...
func NewClient(ks *keystone.Client, cfg Config) (*Client, error) {
	ep, err := ks.Endpoint("orchestration", cfg.Interface, cfg.Region)
	if err != nil {
		return nil, fmt.Errorf("heat: endpoint lookup failed: %w", err)
	}
...
func (c *Client) do(ctx context.Context, req *http.Request) (*http.Response, error) {
	...
	if resp.StatusCode >= 400 {
		return nil, fmt.Errorf("heat: http error: %s", resp.Status)
	}
